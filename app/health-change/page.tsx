...
export default function HealthChangePage() {
  ...
  useEffect(() => { setFromMy(new URLSearchParams(window.location.search).get("from") === "my"); const saved = getFromStorage<HealthChangeSnapshot[]>(STORAGE_KEYS.HEALTH_CHANGE_SNAPSHOTS, sampleSnapshots); setSnapshots(saved); saveToStorage(STORAGE_KEYS.HEALTH_CHANGE_SNAPSHOTS, saved); }, []);
  ...
  return <MobileShell><AppHeader title="나의 건강 변화" showBack backHref={fromMy ? "/my" : "/dashboard"} /><main className="flex-1 overflow-y-auto bg-[#F7FBF8] px-4 pb-24 pt-4">
